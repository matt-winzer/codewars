...
Test.describe("Some tests in random order:", function () {
  ...
  order.forEach(
    function (o) {
      ...
      var missing = "abcdefghijklmnopqrstuvwxyz".split('').filter(function (c) { return str.indexOf(c) < 0 })
      console.log("Is '" + str + "' a pangram ?")
      Test.assertEquals(isPangram(str), exp, str + " is " + ["not", ""][+exp] + " a pangram" + (!!missing ? " (" + missing + " missing)" : ""))
    }
  )
